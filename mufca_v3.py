...
TP_PERCENTILE        = 0.75  # Aggressive mode
SAFE_TP_PERCENTILE   = 0.50  # Safe mode (median)
USE_SAFE_TP          = False  # Active mode flag — toggled by !tpconfig mode
MIN_TP_PCT           = 0.3
MAX_TP_PCT           = 8.0
MAX_HOLD_BARS        = 20
...
def calculate_adaptive_tp(ticker: str, tf: str, side: str, entry: float, current_sl: float) -> float:
    ...
    Uses USE_SAFE_TP flag to switch between aggressive (75th) and safe (50th) percentile.
    """
    history = load_signals_history()
    ...
    mode = "SAFE" if USE_SAFE_TP else "AGGR"
...
def calculate_combined_tp(ticker: str, tf: str, side: str, entry: float, sl: float,
    ...
    if stats["count"] >= 5:
        desc = f"📚 Adaptive {active_pct*100:.0f}th %ile [{mode_label}] | {stats['count']} signals | Avg MFE {stats['avg_mfe']:.2f}%"
    else:
...
def backtest_history(ticker: str, tf: str, num_bars: int = 3000) -> int:
    """
    Populate signal history from historical bars using FIXED R:R 2.0 TP.

    IMPORTANT: Backtest does NOT use adaptive TP. It uses fixed R:R 2.0 to generate
    clean MFE (Max Favorable Excursion) data. This MFE history is then used by
    calculate_adaptive_tp() in LIVE trading to compute dynamic take-profit levels.

    This prevents a circular dependency where adaptive TP needs history,
    but history needs adaptive TP to be generated.
    """
    ...
    try:
        ...
        for idx in range(50, len(df) - 100):
            ...
            for side, sig_ok in [("long", sig_a_long or sig_u_long), ("short", sig_a_short or sig_u_short)]:
                ...
                # FIX #5: Backtest uses FIXED R:R 2.0 to accumulate clean MFE history.
                # Adaptive TP is calculated in LIVE mode only, after history is populated.
...
def check_signals(ticker: str, timeframe: str, st: dict):
    ...
    try:
        ...
        if trade:
            ...
            # FIX: check TP/SL every scan, but increment bar counter only on new bar
            ...
            if hit:
                ...
                if st["bars_in_trade"] >= MAX_HOLD_BARS:
...
def build_embed(ticker, tf, signal_type, price, regime, leverage, confidence,
    ...
    embed.add_field(name="🎯 Take Profit",   value=f"${tp:,.2f} (+{tp_pct:.2f}%)",          inline=True)
...
async def status_cmd(ctx):
    ...
    for ticker in TICKERS:
        for tf in TIMEFRAMES:
            ...
            trade_info = ""
            if trade:
                trade_info = f" | 🎯 {trade['side'].upper()} @ ${trade['entry']:.2f} SL:${trade['sl']:.2f} TP:${trade['tp']:.2f}"
...
async def tpconfig_cmd(ctx, param: str = "", value: str = ""):
    ...
    if not param:
        await ctx.send(
            ...
            f"• Aggressive percentile: **{TP_PERCENTILE*100:.0f}th** (higher profit, lower win rate)\n"
            f"• Safe percentile: **{SAFE_TP_PERCENTILE*100:.0f}th** (lower profit, higher win rate)\n"
    ...
    if param == "mode":
        if value.lower() == "safe":
            USE_SAFE_TP = True
            await ctx.send(f"🛡️ **Safe mode enabled** — TP now uses **{SAFE_TP_PERCENTILE*100:.0f}th percentile** (median). Higher win rate, smaller profit per trade.")
            print(f"[TPCONFIG] Switched to SAFE mode ({SAFE_TP_PERCENTILE*100:.0f}th percentile)")
        elif value.lower() in ("aggressive", "aggr"):
            USE_SAFE_TP = False
            await ctx.send(f"⚡ **Aggressive mode enabled** — TP now uses **{TP_PERCENTILE*100:.0f}th percentile**. Higher profit, lower win rate.")
            print(f"[TPCONFIG] Switched to AGGRESSIVE mode ({TP_PERCENTILE*100:.0f}th percentile)")
        else:
            ...
            for timeframe in TIMEFRAMES:
                ...
                if trades:
                    ...
                    for i, trade in enumerate(trades[-5:], 1):
                        ...
                        lines.append(f"{emoji} #{i} {trade['side'].upper()} | Entry: ${trade['entry']:.2f} → Exit: ${trade['exit']:.2f} | PnL: {trade['pnl_pct']:.2f}% | {trade['result'].upper()}")
        if len(lines) == 1:
...
async def signals_cmd(ctx, ticker: str = "", tf: str = "", side: str = ""):
    ...
    for i, rec in enumerate(records[-15:], 1):
        ...
        lines.append(
            f"{emoji} #{i} Entry: ${rec['entry']:.2f} → Exit: ${rec['exit']:.2f} | "
...
async def tp_cmd(ctx, side: str = "long", ticker: str = "BTC/USDT", tf: str = "1h"):
    ...
    try:
        ...
        lines.append(f"• Take Profit: **${tp:,.2f}** (+{tp_pct:.2f}%)")
...
async def forcerun_cmd(ctx, side: str = "long", ticker: str = "BTC/USDT", tf: str = "1h"):
    ...
    try:
        ...
        embed.add_field(name="Pair",   value=f"**{ticker}**",          inline=True)
        embed.add_field(name="TF",     value=tf.upper(),                inline=True)
        embed.add_field(name="Entry",  value=f"${last_close:,.2f}",     inline=True)
        embed.add_field(name="SL",     value=f"${sl:,.2f}",             inline=True)
        embed.add_field(name="TP",     value=f"${tp:,.2f} (+{tp_pct:.2f}%)", inline=True)
                ...
                if last.get("exit_time"):
                    ...
                    if age < 35:
                        ...
                        await channel.send(
                            ...
                            f"{last['side'].upper()} | Entry: ${last['entry']:.2f} → Exit: ${last['exit']:.2f} | "
